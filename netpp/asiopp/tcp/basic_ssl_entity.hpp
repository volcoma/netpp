    ...
    , config_(config)
{
    context_.set_options(asio::ssl::context::default_workarounds | asio::ssl::context::no_sslv2 |
                         asio::ssl::context::single_dh_use | config.options);
    ...
    {
		context_.set_verify_callback([verify_callback = config.verify_callback, get_peer_callback = config.get_peer_cert_callback](bool preverified,
            ...
            //should be separate callback, because verify callback is not been called every time
            if (get_peer_callback) {
                get_peer_callback(cert.hash) ;
            }
