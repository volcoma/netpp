...
class basic_server : public asio_connector,
                         public std::enable_shared_from_this<basic_server<protocol_type>>
{
public:
    ...
    void on_handshake_complete(std::shared_ptr<socket_type> socket)
    {
        ...
        auto session = std::make_shared<tcp_connection<socket_type, builder_type>>(socket, io_context_);
        auto weak_this = weak_ptr(this->shared_from_this());
