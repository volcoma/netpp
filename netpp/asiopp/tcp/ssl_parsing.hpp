...
inline void log_cert(const ssl_certificate& cert)
{
    ...
    log() << "Certificate Hash (" << cert.hash <<")";

    log() << "-------------------";
}

inline unsigned char *simple_digest(std::string alg, char *buf, unsigned int len, unsigned int *olen)
{
    const EVP_MD *m;
    EVP_MD_CTX *ctx;
    unsigned char *ret;
...
inline std::string get_peer_cert_hash(X509* x509)
{
    std::string hash;
    if (!x509) {
        return hash;
    ...
    ServerCertHash = simple_digest("sha1", server_cert_buf_mem->data, uint32_t(server_cert_buf_mem->length), &ServerCertHashLen);
...
inline bool verify(bool preverified, asio::ssl::verify_context& ctx, ssl_certificate& cert)
{
    ...
    cert.hash = utils::get_peer_cert_hash(x509_cert);
