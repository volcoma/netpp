...
tcp_ssl_server::tcp_ssl_server(asio::io_context& io_context, const protocol_endpoint& listen_endpoint,
							   const std::string& cert_chain_file, const std::string& private_key_file,
							   const std::string& dh_file)
	: tcp_server (io_context, listen_endpoint)
...
void tcp_ssl_server::start()
{
    ...
    auto on_connection_established = [weak_this, socket]() mutable
    {
        ...
        socket->async_handshake(
            asio::ssl::stream_base::server, [weak_this, socket](const error_code& ec) mutable {
                if(ec)
                {
