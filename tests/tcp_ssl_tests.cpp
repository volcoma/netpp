...
void test(bool server, bool client)
{
	...
	try
	{
		...
		if(client)
		{
			auto connector = net::create_tcp_ssl_client(host, std::to_string(port), CERT_DIR "ca.pem");
		...
		if(server)
		{
			auto connector = net::create_tcp_ssl_server(port, CERT_DIR "server.pem", CERT_DIR "server.pem",
														CERT_DIR "dh2048.pem");
